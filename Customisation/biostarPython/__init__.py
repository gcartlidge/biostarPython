...
class ServerSvc:
  ...
  def handleUserPhrase(self, serverReq, errCode, userPhrase):
    try:
      ...
      raise      
  def handleGlobalAPB(self, serverReq, errCode, zoneID, userPhrase):
    try:
      self.stub.HandleGlobalAPB(server_pb2_grpc.server__pb2.HandleGlobalAPBRequest(deviceID=serverReq.deviceID, seqNO=serverReq.seqNO, errCode=errCode, zoneID=zoneID, userPhrase=userPhrase))
    except grpc.RpcError as e:
